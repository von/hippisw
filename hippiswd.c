	...
	char			**argv;
{
	...
	if (((pid = read_pid_file()) != ERROR) && (!force_start)) {
		fprintf(stderr,
				"File %s already exists. Daemon with pid %d possibly\n",
				HIPPISWD_PID_FILE, pid);
		fprintf(stderr,
				"already running. If not remove file %s in working directory.\n",
				HIPPISWD_PID_FILE);
...
restart()
{
	...
	/*
	 * If the binary path starts with a '/' then don't prepend working
	 * directory.
	 *
	 */
